...
class GRULayer():
    ...
    def change_input(self,input,output):
        ...
        self.uNum = np.shape(input)[0]
        self.yvalues = np.zeros(shape=(self.uNum,))  # y
        self.hvalues=np.zeros(shape=(self.uNum,self.hdim))
    def forward(self):
    ...
    def backprop(self,y_G,W_hy_,learning_rate=1e-4,optimizer='Adam'):
        ...
        s_hy = np.zeros(shape=np.shape(W_hy_))
        ...
        r_hy = np.zeros(shape=np.shape(W_hy_))
        ...
        t = self.uNum-1
        while t > 0:
            # delta_y = w*(y_G[t] - self.d[t])*diffsigmoid(y_G[t])
            new=(y_G[t][0]-self.d[t])*diffsigmoid(y_G[t][0])
            delta_h = np.multiply(new,np.transpose(W_hy_))+np.matmul(delta_z_next,np.transpose(self.W_hz_))+np.multiply(
        ...
        # delta_y = w*(y_G[0]-self.d[0])*diffsigmoid(y_G[0])
        new = (y_G[0] - self.d[0]) * diffsigmoid(y_G[0])
        delta_h = np.multiply(new,np.transpose(self.W_hy_))+np.matmul(delta_z_next,
        ...
        if optimizer == 'Adam':
            ...
            self.W_hy_ -= learning_rate * W_hy_
